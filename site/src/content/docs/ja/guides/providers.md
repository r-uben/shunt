...
組み込みの `cursor` プロバイダーは、Cursor 自身の ConnectRPC/protobuf AgentService を通じて、あなたの **Cursor** サブスクリプションに到達します — `kind = "cursor"` のネイティブアダプターが、ストリーミングを含めて Anthropic Messages との間で変換します。ログインとトークンのリフレッシュは `api2.cursor.sh` を使用し、エージェントのターンは Cursor の現在のエージェントホスト（`agentn.global.api5.cursor.sh`）へ HTTP/2 で実行されます。一度ログインしてください。
...
:::note[アダプターが伝えるもの]
アダプターはアシスタントの **テキストと推論** をストリーミングし、クライアントの **ツール** をネイティブな Cursor MCP ツール呼び出しとしてブリッジし（モデルが呼び出したツールは `stop_reason: "tool_use"` を持つ Anthropic の `tool_use` ブロックとして現れます — クライアントがそれを実行して `tool_result` を返すと、shunt はその結果を履歴に含めてターンを再実行します）、**インライン画像**（base64 ソース。URL 画像はスキップ）を転送します。Cursor 自身のエージェント的なファイル/シェルツールは公開されず、リクエストが宣言したツールのみが使われます。
:::

**モデル id とエージェントモード。** プレフィックスが Cursor のエージェントモード（Agent / Plan / Ask）を選択し、サフィックスが Cursor のモデル id です。`cursor-agent models` が表示する表示名ではなく、**ワイヤー** id を使用してください。Auto は `default` です（`cursor:auto` にルーティングすると `Unknown model ID: auto` で失敗します）。名前付きモデル（例: `cursor:gpt-5.2`）はそれを許可する有料プランが必要で、無料プランは `cursor:default` に限定されます。
...
:::note[オーバーライド]
`SHUNT_CURSOR_BASE_URL` はログイン/リフレッシュのエンドポイントを、`SHUNT_CURSOR_AGENT_BASE_URL` はエージェントホスト（HTTPS の `cursor.sh` ホストである必要があります）を、`SHUNT_CURSOR_AUTH_FILE` は認証情報のパスを、`SHUNT_CURSOR_CLIENT_VERSION` は `x-cursor-client-version` ヘッダーをオーバーライドします（Cursor が古いクライアントバージョンを拒否し始めた場合、再ビルドせずに更新できます）。`cursor_oauth` プロバイダーは HTTPS 経由の Cursor ホストに固定されます — `base_url` をオリジン外へ向けることは拒否され、ベアラートークンが漏洩しないようになっています。
:::
