...
ダッシュボードは、`auth = "claude_oauth"` または `auth = "chatgpt_oauth"` の provider について、アカウントストアのメタデータと現在の状態を表示します。Claude の行には、上流の応答から観測した 5 時間・共有 7 日・`7d_oi` の使用率が、ステータスやクールダウン状態とともに表示されます。上流がリセット時刻を報告している場合は、各ウィンドウのセルにリセットまでの残り時間（例: `3d 4h`）も表示され、ホバーすると絶対リセット時刻を確認できます。Codex の行には、`x-codex-*` 応答ヘッダーで報告される 5 時間・7 日のウィンドウが表示されます。未対応のウィンドウは無視され、Codex には相当するものがないため `7d_oi` は `—` のままです。Codex の使用量は、Claude の使用量とまったく同じように、クォータを考慮したアカウント選択にも供給されます（issue #195）。
