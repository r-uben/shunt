...
- **C. Codex WS:流中途失败的恢复(已跟踪:[#46])。** 流式开始*之前*的 WS 失败会透明地回退到 HTTP,但*流中途*的失败以错误 SSE 事件的形式浮出,而不是回退(`src/adapters/responses.rs:92-135`)。可考虑恢复/重放,让轮次中途掉线的套接字降级为 HTTP 而不是报错。
