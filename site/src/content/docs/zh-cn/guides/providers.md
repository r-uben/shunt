...
内置的 `cursor` 提供方通过 Cursor 自己的 ConnectRPC/protobuf AgentService 访问你的 **Cursor** 订阅 —— `kind = "cursor"` 原生适配器在其与 Anthropic Messages 之间双向转换,含流式传输。登录与令牌刷新使用 `api2.cursor.sh`;agent 轮次通过 HTTP/2 发往 Cursor 当前的 agent 主机(`agentn.global.api5.cursor.sh`)。登录一次:
...
:::note[适配器承载的内容]
适配器会流式传输助手的 **文本与推理**,把客户端的 **工具** 桥接为原生的 Cursor MCP 工具调用(模型调用的工具会呈现为带 `stop_reason: "tool_use"` 的 Anthropic `tool_use` 块 —— 你执行它并把 `tool_result` 送回,shunt 便会带着该结果在历史中重新运行该轮次),并转发 **内联图像**(base64 源;URL 图像会被跳过)。Cursor 自身的智能体式文件/Shell 工具不会被暴露,仅使用请求所声明的工具。
:::

**模型 id 与 agent 模式。** 前缀用于选择 Cursor 的 agent 模式(Agent / Plan / Ask),后缀是 Cursor 模型 id。请使用 **wire** id,而非 `cursor-agent models` 显示的展示名:Auto 是 `default`(路由 `cursor:auto` 会以 `Unknown model ID: auto` 失败)。命名模型(例如 `cursor:gpt-5.2`)需要有权使用它的付费计划,免费计划仅限 `cursor:default`。
...
:::note[覆盖项]
`SHUNT_CURSOR_BASE_URL` 覆盖登录/刷新端点,`SHUNT_CURSOR_AGENT_BASE_URL` 覆盖 agent 主机(必须仍为 HTTPS 的 `cursor.sh` 主机),`SHUNT_CURSOR_AUTH_FILE` 覆盖凭据路径,`SHUNT_CURSOR_CLIENT_VERSION` 覆盖 `x-cursor-client-version` 头部(若 Cursor 开始拒绝过时的客户端版本,可无需重新构建即予以提升)。`cursor_oauth` 提供方被固定到某个 Cursor 主机并使用 HTTPS —— 将 `base_url` 指向非同源地址会被拒绝,以防 bearer token 泄露。
:::
