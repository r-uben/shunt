...
- `site/src/content/docs/` — update the affected getting-started/guide/reference page when user-facing behavior, config keys, endpoints, or CLI changes (`getting-started/` for installation/quickstart, `guides/` for how-tos, `reference/` for config/endpoints/CLI/troubleshooting).
- `wiki/` — generated by memex; do **not** hand-edit and do not include it in a routine code PR. Regenerate separately when its source material changes.

A change that genuinely needs no doc update is fine — but confirm each surface
above was considered rather than skipped by default. Docs are English-only, as
with code.
